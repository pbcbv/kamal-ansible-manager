...
- **`playbook.yml`** — the main playbook. Runs against the `webservers` group with `gather_facts: false` (facts are gathered explicitly by the `wait_for_connection` role instead, so the play can connect to freshly-booted hosts before they're fully ready). Roles run in this order, and the order matters:
  ...
  4. `deploy_user` — creates the `{{ deploy_user_name }}` user (default `deploy`) and adds it to the `docker` group. Who may SSH in as that user is decided by `deploy_user_authorized_keys`: set it and the playbook writes exactly that list, empty (the default) and it copies root's `authorized_keys` as before. Either way the file is rewritten on every run, so it is managed state, not a place to append a key by hand. Runs before `security` so root SSH access is still guaranteed while the deploy user is being provisioned.
  5. `firewall` — UFW: deny incoming / allow outgoing, limit 22, allow 80/443.
  6. `security` — hardens `sshd_config` (disables password login, root password login) and configures unattended-upgrades auto-reboot.
  7. `geerlingguy.swap` — external role (from `requirements.yml`) that configures swap.
  8. `reboot_if_needed` — reboots if `/var/run/reboot-required` exists.
...
- Tunable variables (`security_autoupdate_reboot`, `security_autoupdate_reboot_time`, `deploy_user_name`, `deploy_user_authorized_keys`, and any `geerlingguy.swap` defaults like `swap_file_size_mb`) are set under `vars:` in `playbook.yml`.
